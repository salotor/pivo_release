...
screen sdl_achvlist_section(engine):
    ...
    for route in sdl_achv_section_to_routes[engine.get_cur_section()]:
        ...
        if engine.get_cur_route() == route:
            add route.get_icon_active() xcenter 1420 ycenter (165 + 117 * sdl_achv_y_shift)
        else:
            imagebutton xcenter 1420 ycenter (165 + 117 * sdl_achv_y_shift):
                hover_sound sdl_achv_click
                ...
                action [Function(engine.set_hovered_route, None), Function(engine.run_func_out), Function(engine.set_cur_route, route), Function(engine.run_func_in), Show("sdl_achv_sprite", Dissolve(0.5), route.get_sprite_who(), "pi_smile" if route == sdl_achv_route_me_neu else "normal" if route == sdl_achv_route_me_7dl else "smile", route.get_sprite_clothes())]
        $ sdl_route_count += 1
...
screen sdl_achv_route(engine):
    ...
    if persistent.pivo_completed:
        imagebutton xcenter 500 ycenter 70:
            hover_sound sdl_achv_click
            idle ("sdl_achv_stat_inactive")
            hover ("sdl_achv_stat_active")
            hovered [Function(engine.set_hovered_icon, "sdl_achv_stat" if not sdl_achv_show_stats else "sdl_achv_stat_off")]
            unhovered [Function(engine.set_hovered_icon, None)]
            action [Function(engine.set_hovered_icon, None), ToggleVariable("sdl_achv_show_stats")]
        ...
        if (achv.get_persistent() in sdl_achv_hidden) and not (getattr(persistent, achv.get_persistent())):
            ...
            if getattr(persistent, achv.get_persistent()):
                ...
                imagebutton xcenter 780 ycenter (165 + 64 * sdl_achv_count):
                    ...
                    unhovered [Function(engine.set_hovered_achv, None), Show("sdl_achv_sprite", Dissolve(0.5), engine.get_cur_route().get_sprite_who(), "pi_smile" if engine.get_cur_route() == sdl_achv_route_me_neu else "normal" if engine.get_cur_route() == sdl_achv_route_me_7dl else "smile", engine.get_cur_route().get_sprite_clothes())]
