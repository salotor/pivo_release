...
init 1:
    ...
    image sdl_uv_chibi = get_image_7dl("gui/menu_elem/misc/uvao.png")

    $ alt_binder_text = "{image=sdl_achv_puzzle}\n\nПОЗДРАВЛЯЕМ!\n\nВы наконец собрали пазл-сердечко, и теперь вам доступны для прохождения:\n\n- ветка “ответов” в руте Семёна-Одиночки;\n- концовки-“осколки памяти”;\n- послесловия для некоторых РФ-концовок (доступны также из ачивлиста);\n- бонусные разделы в “Старой Дороге”"

    $ alt_completed_text = "{image=sdl_achv_puzzle}\n\nПОЗДРАВЛЯЕМ!\n\nВы наконец прошли модификацию, и теперь вам доступны небольшие приятные бонусы:\n\n- статистика прохождения мода по нажатию на логотип в главном меню;\n- расширенная статистика по каждой концовке в ачивлисте"

    $ alt_changelog_text = "{size=35}{u}Список изменений в версии 1.0:{/u}{/size}\n- доступны для прохождения 6-7 дни Финального рута с концовками;\n- добавлена 21 новая иллюстрация и множество их вариаций;\n- добавлен новый оригинальный трек от April Rain;\n- весь список изменений {a=https://telegra.ph/Spisok-izmenenij-obnovleniya-10-03-19}здесь{/a}.\n\nЧтобы быть в курсе последних новостей, подписывайтесь на наши ресурсы: {a=https://vk.com/bl_7dl}группа ВКонтакте{/a}, {a=https://t.me/sevendaysofbeer}канал в Телеграме{/a}, {a=https://discord.gg/qd8drECDHp}сервер в Дискорде{/a}\n\nСтраница со схемами и гайдами для прохождения в {a=https://vk.com/page-128046483_52527656}ВК{/a} или {a=https://t.me/sevendaysofbeer/892}ТГ{/a}"
...
screen menu_7dl():
    imagemap at left_menu_7dl(0.1, 0.7):
        ...
        if not persistent.pivo_default_7dl:
            ...
            if persistent.pivo_completed:
                hotspot (1673, 81, 214, 120):
                    hover_sound get_sfx_7dl("ach_list/achv_click_7dl.ogg")
                    action [Hide("menu_7dl", transition=Dissolve(0.2)), Hide("settings_7dl", transition=Dissolve(0.2)), Hide("contacts_7dl", transition=Dissolve(0.2)), Hide("media_7dl", transition=Dissolve(0.2)), Jump("alt_7dl_stats")]
        else:
            if persistent.waifu_7dl == 'un':
            ...
            hotspot (170, 874, 137, 39):
                hover_sound get_sfx_7dl("ach_list/achv_click_7dl.ogg")
                action [Hide("settings_7dl", transition=Dissolve(0.2)), Hide("contacts_7dl", transition=Dissolve(0.2)), Hide("media_7dl", transition=Dissolve(0.2)), Show("alt_quit", transition=Dissolve(0.2))]
            ...
            elif persistent.pivo_completed:
                hotspot (1673, 81, 214, 120):
                    hover_sound get_sfx_7dl("ach_list/achv_click_7dl.ogg")
                    action [Hide("menu_7dl", transition=Dissolve(0.2)), Hide("settings_7dl", transition=Dissolve(0.2)), Hide("contacts_7dl", transition=Dissolve(0.2)), Hide("media_7dl", transition=Dissolve(0.2)), Jump("alt_7dl_stats")]
...
screen settings_7dl(embedded=False):
    ...
    $ pos_no += 1
    if not persistent.stream_mode_7dl:
        textbutton "Режим стримера: выкл." xpos xpos_val ypos ypos_val+0.0455*pos_no:
            style "log_button"
            text_style "alt_settings_textbutton"
            hover_sound get_sfx_7dl("ach_list/achv_click_7dl.ogg")
            hovered Show("settings_description_7dl", Dissolve(0.2), "Включить замену треков, которые блокируются на ютубе.", embedded)
            unhovered [Hide("settings_description_7dl", transition=Dissolve(0.2))]
            action [SetField(persistent,'stream_mode_7dl',True), Show("alt_confirm_reload", Dissolve(0.5), False)]
    else:
        textbutton "Режим стримера: вкл." xpos xpos_val ypos ypos_val+0.0455*pos_no:
            style "log_button"
            text_style "alt_settings_textbutton"
            hover_sound get_sfx_7dl("ach_list/achv_click_7dl.ogg")
            hovered Show("settings_description_7dl", Dissolve(0.2), "Выключить замену треков, которые блокируются на ютубе.", embedded)
            unhovered [Hide("settings_description_7dl", transition=Dissolve(0.2))]
            action [SetField(persistent,'stream_mode_7dl',False), Show("alt_confirm_reload", Dissolve(0.5), False)]
...
label random_bg_7dl:
    ...
    if persistent.alt_changelog != alt_release_no:
        show screen alt_changelog(alt_changelog_text)
        $ persistent.alt_changelog = alt_release_no
        $ renpy.pause(0.1)
    if persistent.alt_show_binder_msg:
        show screen alt_changelog(alt_binder_text)
        $ persistent.alt_show_binder_msg = False
        $ renpy.pause(0.1)
    if (not persistent.alt_completed_msg_shown) and persistent.pivo_completed:
        show screen alt_changelog(alt_completed_text)
        $ persistent.alt_completed_msg_shown = True
        $ renpy.pause(0.1)
...
label alt_7dl_stats:
    $ endings, route = sdl_ending_counter()
    if persistent.got_lucky_7dl == None:
        $ persistent.got_lucky_7dl = 0
    if persistent.got_lucky_7dl % 10 == 1 and persistent.got_lucky_7dl % 100 != 11:
        $ raz = "раз"
    elif persistent.got_lucky_7dl % 10 in (2, 3, 4) and persistent.got_lucky_7dl % 100 not in (12, 13, 14):
        $ raz = "раза"
    else:
        $ raz = "раз"
    scene black with dissolve
    show alt_credits "Топ-3 концовки:\n{image=[endings[0][0]]} Прохождений: [endings[0][1]]\n{image=[endings[1][0]]} Прохождений: [endings[1][1]]\n{image=[endings[2][0]]} Прохождений: [endings[2][1]]\n\nТоп-1 рут:\n{image=[route[0]]} Среднее число прохождений концовок: [route[1]]\n\n{image=sdl_uv_chibi} Юля встретилась в меню [persistent.got_lucky_7dl] [raz]":
        xalign 0.5
        yalign 0.5
    $ renpy.pause(40)
    jump main_menu_7dl
