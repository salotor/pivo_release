...
init python:
    ...
    class sdl_repl_Engine:
        ...
        def get_cur_title(self):
            if self.cur_route != None:
                return self.cur_route.get_title()
            else:
                return self.cur_section.get_title()

        def get_cur_color(self):
            if self.cur_route != None:
                return self.cur_route.get_color()
            else:
                return self.cur_section.get_color()

        def get_cur_tag(self):
            return self.cur_day.get_title()
    ...
    class sdl_repl_Section:
        def __init__(self, title, color, route_arr, day_arr, available=True):
            self.title = title
            self.color = color
    ...
    # Открыта хотя бы одна концовка в руте
    def sdl_achv_route_access(cur_route):
        gl = globals()
        for var in gl:
            if gl[var] == cur_route:
                for achv in eval(var.replace("repl", "achv_array")):
                    if getattr(persistent, achv.persistent):
                        return True
##\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ЭКРАНЫ\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
# Начальный экран
...
screen sdl_repl_section_selector(engine):
    ...
    if (engine.get_cur_route() != None) or ((engine.get_cur_section() != None) and (engine.get_cur_section().get_route_arr() != None)):
        ...
        text engine.get_cur_title()[:25]:
            style "sdl_repl_font_medium"
            color engine.get_cur_color()
            xalign 0.456
            xanchor 1.0
            ypos 0.182
...
screen sdl_repl_route(engine):
    ...
    if (engine.get_cur_route() != None):
        ...
        for day in engine.get_cur_route().get_day_arr():
            if day.get_label_arr()[0].get_title() == "Selector" and not sdl_achv_route_access(engine.get_cur_route()):
                text day.get_title():
                    style "replays_text_locked"
                    xpos 0.544
                ...
                textbutton day.get_title():
                    ...
                    xpos 0.544
                    ypos (0.187 + 0.0485 * sdl_day_count)
                    action [(SetVariable("cur_replay", day.get_label_arr()[0].get_replay()), Start("sdl_replay") if persistent.pivo_default_7dl else Jump("sdl_replay")) if day.get_label_arr()[0].get_title() == "Selector" else Function(engine.set_cur_day, day)]
            $ sdl_day_count += 1
    else:
        $ sdl_day_count = 0
