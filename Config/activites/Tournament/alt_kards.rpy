...
label alt_day2_cards_tournament:
    ...
    if alt_day2_dv_ultim == 2:
        ...
        "Я улыбнулся девочке."
        me "Привет. Похоже, мы с тобой соперники в этом раунде!"
        show un serious pioneer with dspr
        "Лена не ответила, лишь бросила на меня какой-то непонятный холодный взгляд."
        th "Нервничает? Или во время соревнований в ней просыпается какая-то другая Лена, не ведающая пощады?"
        "Решив не ломать голову над этим, я сел напротив неё и пожелал удачи. {w}Снова без ответа."
        th "Хм. Ну и ладно."
    else:
...
label alt_day2_poker_rules_reading:
    ...
    "- {b}Старшая карта{/b} (англ. {i}high card{i}): ни одна из нижеописанных комбинаций,{nw}"
...
label alt_day2_demo_play_intro_new:
    ...
    if alt_day2_dv_ultim == 2:
        me "Ходи первая."
        "Предложил девочке я."
        "И Лена, не выдавая лицом никаких эмоций, потянулась за картой."
    return
...
label alt_day2_demo_play_me_defend_1_new:
    ...
    if alt_day2_dv_ultim == 1:
        un "Т-ты будешь…"
    if alt_day2_dv_ultim == 2:
        "Лена смотрела на мои карты и чего-то ждала."
...
# ------------------------------------------------- ДИАЛОГИ
    if alt_my_rival_1_tour.take == 'un':
        if alt_day2_dv_ultim == 2:
            ...
            "Чуда не произошло. Снова холодное молчание."
        else:
...
label alt_day2_participate_new:
    ...
    while alt_table_no <= 4:                                                        # перебираем столы от 1 до 4
        if not alt_day2_detour_1_tour:                                              # если НЕ пропуск 1 тура
            $ sitting_at_table,gambler_upper,gambler_lower,must_taunt = alt_declare_tournament_tables(alt_table_no)  # рассадка, игроки, таунты — по номеру стола
            "%(sitting_at_table)s{w=.5}{nw}"
            call show_tournament_table                                                  # переход по метке — вызов очередной фишки
            extend " %(gambler_upper)s{w=.5}{nw}"                                       # выводим в окно имя верхнего игрока
            call show_tournament_table                                                  # переход по метке — вызов очередной фишки
...
# ---------------------------------------------------- ДИАЛОГИ
    if alt_my_rival_1_tour.take == 'un':
        ...
        if alt_day2_dv_ultim == 1:
            ...
            "Лена обдала меня ещё одним холодным взглядом."
            play music music_list["you_lost_me"] fadein 2
            un "Поддаёмся?"
            "Я изобразил искреннее удивление. Справедливости ради, я на самом деле был удивлён."
            me "Нет-нет, ты заслуженно победила!"
            un "Только это ничего не изменит."
            hide un with dissolve
            "Загадочно пробормотала она и резко встала, оставив меня чесать в затылке."
        ...
        "Я собрал карты со стола и сложил в одну стопку."
        if not alt_day2_dv_ultim == 2:
            show un angry2 pioneer with dspr
        ...
        if alt_day2_dv_ultim == 2:
            "Я бросил карты на стол и взглянул на Двачевскую."
            "На её лице была смесь облегчения и какого-то злобного торжества."
            th "Ну, хотя бы не злорадствует перед всеми…"
...
# ---------------------------------------------------- ДИАЛОГИ
    if alt_my_rival_1_tour.take == 'un':
        ...
        if lp_un >= 6 and not alt_day2_detour_semifinal and not alt_day2_dv_ultim == 2:
            menu:
            ...
            un "Я проиграла…"
            "Тихонько пробормотала она."
            "А мне даже стало немножко стыдно. Но ненадолго."
            "Потому что в следующую секунду она бросила на меня ещё один недобрый взгляд и, не говоря ни слова, ушла."
        $ lp_un -= 1
        $ lp_dv += 1
        "Я улыбнулся."
        me "Спасибо за игру!"
...
# ====================================================== Пропускаем часть диалога, если "выиграли" 1 тур скипом
        if not alt_day2_detour_semifinal:
            ...
            if alt_result_dv_1_tour == 4:                                                       # Дваче в другом полуфинале
                ...
                th "Чего не скажешь об Алисе. Значит, это будет лёгкая победа."
...
# ====================================================== Пропускаем часть диалога, если "выиграли" 1 тур скипом
        ...
        if alt_day2_dv_ultim == 1:
            ...
            "Алиса пыхтела, жгла меня взглядом, сжимала и разжимала кулаки — словом, достойно переносила своё поражение."
            "Я решил тактично молчать, чтобы Двачевская не взорвалась от гнева и не забрызгала окружающих."
            show dv rage pioneer2 with dspr
            "И, судя по всему, это было правильным решением: Алиса, хоть и бросила на меня взгляд, наполненный желанием мгновенной смерти, всё же встала и ушла к остальным зрителям."
...
# ---------------------------------------------------- ДИАЛОГИ
    if alt_my_rival_semifinal.take == 'un':
        ...
        if alt_day2_dv_ultim == 2:
            show un normal pioneer with dspr
            "Она не смотрела на меня, но я всё равно ощущал какую-то неприязнь."
            "Насколько я мог понять, Лена не была экстравертом, и такое отстранённое поведение вполне укладывалось в её образ, но…"
            "Я тяжело вздохнул."
            "Меня не покидало чувство, что это всё не просто так."
        else:
            show un shy pioneer with dspr
            me "Удачи нам обоим?"
            show un smile pioneer with dspr
...
# ---------------------------------------------------- ДИАЛОГИ;
    if alt_my_rival_semifinal.take == 'un':
        ...
        if alt_day2_dv_ultim == 2:
            ...
            show un angry2 pioneer with dspr
            "Лена прищурилась."
            th "Да что ей опять не так?"
            un "На секунду даже захотелось посмотреть, как Алиса выполнит свою угрозу."
            me "Пардон?"
            show un normal pioneer with dspr
            "Лена не ответила, просто смотрела на меня."
            "А мне отчего-то очень захотелось в мельчайших деталях рассмотреть столовую, лишь бы не встречаться с ней глазами."
        elif alt_day2_dv_ultim == 1:
            $ lp_un += 1
        ...
        if alt_day2_dv_ultim == 1:
            ...
            "На лице Алисы застыло выражение мстительного удовольствия."
            "И это не предвещало мне ничего хорошего."
        else:
            th "Как же хорошо, что у меня хватило ума не соглашаться на это дурацкое «пари»."
            "Я бросил взгляд на рыжую."
            th "Надеюсь она успокоится и не будет создавать мне проблем."
...
# ---------------------------------------------------- ДИАЛОГИ
    if alt_my_rival_semifinal.take == 'un':
        stop music fadeout 2
        if alt_day2_dv_ultim == 2:
            th "Победа! Победа! Вместо обеда!"
            "Кто-то крикнул «молодец!», кто-то хлопнул по плечу…"
            "Одна лишь Лена, словно позабыв все мантры про дружбу и плечо товарища, открыто проявляла своё недовольство."
            "Не поздравила с победой, не пожала руку. {w}Даже сухим и коротким кивком не удостоила своего соперника."
            "Впрочем, меня это перестало волновать довольно быстро, ведь на носу у меня был финал!"
        else:
...
# ****************   НОВЫЕ  СОПЕРНИКИ   ******************************************
        if alt_my_rival_final.take == 'un':
            ...
            rival = CardGameRivalWiseUsual(mz_avatar_set, u"Женя", alt_day2_gambler_behavior, alt_day2_gambler_skill)           # Женя играет рандомно, как в классике
# ************************************************************************************
...
# ---------------------------------------------------- ДИАЛОГИ
    if alt_my_rival_final.take == 'un':
        ...
        if not alt_day2_dv_ultim == 2:
            ...
            show dv sad pioneer2 far at fright with dissolve
            pause(0.3)
            ...
            show un normal pioneer with dissolve
            me "Поздравляю с победой."
            "Сказал я как можно радостнее."
            "Лена не ответила, всё ещё не отрывая своего взгляда от стола, на котором лежала её победная комбинация."
            ...
            show un shy pioneer:
                ...
                linear 0.5 xpos 0.5
            with dissolve
            "А когда весь зал взорвался апплодисментами, вздрогнула и покраснела."
            dreamgirl "Не ожидала своей победы?"
            th "Сдаётся мне, она меньше всех присутствующих думала о победе."
            dreamgirl "Почему же?"
            th "Э-э-э… {w}Да посмотри ты на неё. Она ж эталон неуверенности в себе."
            dreamgirl "Ну, может ты и прав. Похоже она и правда в шоке от своего триумфа."
            th "Угу."
            play music music_list["you_lost_me"] fadein 2
            "Согласился я, наблюдая за тем, как к Лене подбегают разные пионеры и поздравляют её с победой."
            "Как дрожит её рука, свободная от рукопожатий восхищённых товарищей по отряду."
            "Как из уголка её глаза катится небольшая слезинка…"
            show un sorrow pioneer with dspr
            th "Стоп, что?"
            "Я думал, что она просто стесняется, но теперь увидел, как дрожит её нижняя губа."
            th "Она же сейчас разревётся!"
            "Я оттолкнул радостно гомонящего Электроника и подскочил к Лене."
            me "Всё в порядке?"
            "Спросил я тихонько, чтобы услышать меня могла только она."
            show un silent_angry pioneer with dspr
            "И чуть не отскочил, когда она подняла лицо и посмотрела на меня."
            "То, что я ошибочно принял за расстройство, оказалось гневом."
            "И этот гнев был направлен прямиком на меня!"
            show un angry pioneer with dspr
            un "Не подходи ко мне! Ты всё испортил!"
            me "Ч-что… {w}Я…"
            show un angry2 pioneer with dspr
            un "Вот зачем ты это сделал? Скажи!"
            "Я не знал, что на это ответить."
            th "Зачем я проиграл в финале? Очевидно, что сделал я это не специально."
            th "Зачем я подошёл её успокоить? Да потому что мне не всё равно."
            th "Зачем я…"
            hide un with moveoutright
            "Пока я думал над тем, на какой вопрос я должен дать ответ, Лена вскочила и быстрым шагом направилась к выходу."
            mi "Лена, ты куда? А как же награждение?"
            "Из ниоткуда выпрыгнула Мику и, не слушая слабых отговорок Лены, потащила её на фотосессию."
            stop music fadeout 4
            "Вспышка Лены прошла незамеченной: окружающие были всецело погружены в обсуждение перипетий турнира."
            "И так же, как и я недавно, принимали её красное лицо за признак смущения."
            with fade
            "Я облокотился о колонну чуть поодаль от основной толпы и размышлял."
            show un smile pioneer far at left with dissolve
            "А Лена уже успокоилась, иногда даже улыбалась."
            show un sorrow pioneer far at left with dspr
            "Я бы похвалил её актёрскую игру, если бы спина девочки не деревенела каждый раз, когда я попадался ей на глаза."
            hide un with dissolve
            th "Тяжело…"
        stop music fadeout 3
        ...
        with dspr
        mt "Так! О каких ещё похабностях тут речь?"
        show us shy pioneer far at flinch
        show mi shy pioneer far at flinch
        show un shy pioneer far at flinch
        "Девочки вздрогнули и покраснели."
        show mt normal pioneer with dspr
        mt "Шурик просто…"
        "Она не подобрала какой-то характеристики и просто махнула рукой."
        mt "…какой есть. {w}Ему же хуже — пропустит самое интересное!"
...
# ---------------------------------------------------- ДИАЛОГИ
    if alt_my_rival_final.take == 'un':
        if alt_day2_dv_ultim == 2:
            show un normal pioneer with dissolve
            un "С победой."
            "Сухо бросила она и встала из-за стола."
            me "И это всё?"
            show un serious pioneer with dspr
            "Лена вскинула бровь."
            un "А что, маловато, да? Может и мою грудь хочешь потрогать?"
            show un snide_smile pioneer with dspr
            un "Ты же у нас победитель! Берёшь, что пожелаешь! {w}Ну так что, будешь?"
            "Я сидел, не зная как реагировать и надеялся на то, что никто её слов не услышал."
            me "Постой! Я не это имел в виду!"
            show un angry2 pioneer with dspr
            un "А что, моя нравится меньше?"
            me "Да нет, тоже нравится… {w}Тьфу ты, не об этом речь!"
            dreamgirl "Чувак, лучше помолчи. Она явно не в себе."
            th "Но я ведь должен как-то оправдаться!"
            dreamgirl "Пока что у тебя получается крайне плохо."
            th "Так ты не лезь, может и мысля хорошая придёт!"
            dreamgirl "Поздно."
            hide un with dissolve
            "Лена хмыкнула и отвернулась. И не говоря больше ни слова, ушла к остальным."
            scene bg int_dining_hall_sunset with dissolve
            "Так и не придумав, как объяснить девочке всю ситуацию, я подошёл к доске, на которой Сыроежкин уже написал имя победителя — моё!"
        else:
            show un shy pioneer with dissolve
            un "К-кажется, в-всё."
            play music music_7dl["take_my_hand"] fadein 3
        ...
        if alt_day2_dv_ultim == 2:
            dv "Ты выиграл себе немного времени."
            th "Звучит зловеще."
        elif alt_day2_dv_ultim == 1:
        ...
        hide dv with dissolve
        if alt_day2_dv_ultim == 2:
            "Она как-то очень загадочно улыбнулась и растворилась в толпе."
        else:
    ...
    if alt_spr_my_rival == 'un':
        if alt_day2_current_rout_status == 1:                   # если Семён выиграл первый раз
            ...
            if not alt_day2_dv_ultim == 2:
                ...
                show un angry2 pioneer at cright with dissolve
                "Лена поджала губы и выпрямила спину."
                th "Ого, кто-то настроен серьёзно!"
            ...
            if not alt_day2_dv_ultim == 2:
                ...
                #реж
                "Лена стойко перенесла поражение в этом раунде."
                "А я, воспользовавшись тем, что Лена на меня не смотрит, украдкой выдохнул."
                dreamgirl "Это что, облегчение? Я-то думал, что ты не откажешься проиграть."
                th "Ну не знаю…"
            ...
            if alt_day2_gambler_behavior == 'succumb':
                ...
                if not alt_day2_dv_ultim == 2:
                    ...
                    "Правда, как-то комментировать это она не стала."
                    dreamgirl "По-моему, тут комментарии излишни."
                    th "Не понял."
                    dreamgirl "Да она наверянка в шоке от того, насколько ты плох в картах!"
                    th "Иди ты!"
            else:
                ...
                if not alt_day2_dv_ultim == 2:
                    ...
                    "Видимо, пора пересмотреть свои взгляды на эту девочку."
            ...
            if alt_day2_gambler_behavior == 'succumb':          # если "поддавки"
                ...
                if not alt_day2_dv_ultim == 2:
                    ...
                    show un serious pioneer at cright with dissolve
                    "Лена вскинула брови, явно не ожидая такого исхода."
                    me "Молодец. Отлично сыграно!"
                    show un normal pioneer with dspr
                    "Девочка никак не отреагировала на похвалу."
                    dreamgirl "Там вроде уголок рта дёрнулся."
                    th "Я не заметил…"
            else:
                ...
                if not alt_day2_dv_ultim == 2:
                    ...
                    th "Помогло ведь!"
                    dreamgirl "Ты о чём?"
                    th "Ну… {w}Она губы поджала и сразу победила!"
                    "Внутренний голос хмыкнул."
                    dreamgirl "Ещё какое оправдание придумаешь?"
# Славя
...
# Алиса
        ...
        if alt_day2_current_rout_status == 1:                   # если Семён выиграл первый раз
            ...
            if not alt_day2_dv_ultim == 2:
                ...
                #реж
                "Алиса злобно сощурилась."
                dv "Вот она — расплата близко!"
                "Я фыркнул."
                me "Рано радуешься. Я даже не разогрелся!"
                dreamgirl "Думаешь твой блеф прокатит?"
                th "Посмотрим."
                "Алиса как-то разочарованно хмыкнула — явно ждала другой реакции."
            ...
            if not alt_day2_dv_ultim == 2:
                ...
                show dv normal pioneer2 at cright with dissolve
                "Двачевская явно выдохнула."
                me "Не задирай нос, рыжая!"
                show dv laugh pioneer2 with dspr
                "Она расхохоталась."
                dv "Задирать не буду. А вот кое-что кое-кому надрать после победы я точно готова!"
                th "Она про мою задницу?"
                dreamgirl "Ага."
                me "Фу, как вульгарно!"
                show dv angry pioneer2 with dspr
                "Ляпнул я, не подумав. Точнее, забыв о том, что сделал немного ранее на крыльце."
                show dv rage pioneer2 with dspr
                dv "Да ты совсем офигел, новичок!"
                "Она сжала кулаки и приготовилась побеждать."
...
# Мику
        ...
        if alt_day2_current_rout_status == 1:                   # если Семён выиграл первый раз
            ...
            "Услышав впервые, я не придал этому особого значения и пропустил мимо ушей, однако…"
            ...
            mi "Ну держись, Сенечка! Сейчас я выиграю тебя!"
            show mi smile pioneer at cright with dspr
            me "Ага, удачи."
...
label alt_day2_1_tour_analizer:
    ...
    #результат, игроки, высказывания — по номеру стола
        ...
        "%(results_at_table)s{w=.5}{nw}"                                                # оглашаем результат за столом
        call show_tournament_table                                                      # переход по метке — вызов очередной фишки
