label alt_day0_start_d:
    ...
    "Каждый раз я хватался за неё в отчаянной надежде спастись, но каждый раз просыпался."
    scene anim prolog_5
    with dissolve
    "Меня снова ждала моя бесцветная реальность."
    "Холодная, молчаливая и вечная."
    "Я почти смирился с тем, что мне не суждено было узнать, куда меня приведёт незнакомка."
    scene
    ...
    "В тот самый момент, когда «сто вторая» маршрутка хлопнула за спиной дверью и умчалась прочь, оставляя меня наедине с памятью."
    ...
    "Но всё равно панические атаки накрывали каждый раз, когда я видел в новостях сообщения об автомобильных катастрофах."
    nvl clear
    $ renpy.show("uvao_d1", at_list = [left], what = Notch("uvao_d1"))
    with dspr
    ...
    "Потому я и остался здесь, наедине с компьютером, одиночеством и почти забытым прошлым."
    nvl clear
    ...
    "Звала с собой."
    "Но почему-то дело никогда не заходило дальше моего согласия. {w}Сегодня же всё было иначе."
    "Так что…"
    pause(2)
    menu:
        "Пойду, милая":
...
label alt_day0_start_h:
    ...
    "Мой отчим привил мне тягу к этой Высшей Справедливости — и это было его самым существенным вкладом в моё воспитание."
    "И несмотря на подлость отца, на предательство — я продолжал глупо верить в совесть, в справедливость для всех."
    "Не умел иначе."
    nvl clear
    ...
    "Я заметил его ещё на улице, когда тот неуверенным шагом приближался к месту моей работы."
    "Если бы я знал, что увижу, войдя следом за ним, то я придержал бы дверь, заглушил бы своё присутствие…"
    "Но я увидел перед своим лицом дуло пистолета ещё до того, как начался мой рабочий день."
    "Ему было плевать на мой распорядок. Плевать на закон. Он хотел выпить."
    "Чёрт бы побрал этот дурацкий день…"
    stop ambience
    play sound sfx_7dl["makarych"] fadein 0
    scene bg ext_entrance_night_clear_7dl
    show prologue_dream
...
label alt_day0_start_l:
    ...
    "До конца года оставалось немногим меньше месяца, до поездки в Германию — полутора, когда давным-давно мёртво молчащий контакт разродился «приветом»."
...
label alt_day0_start_am:
    play music music_7dl["unseen"] fadein 1
    scene bg ext_camp_entrance_day at sepia_s
    with fade3
    "Где-то там, за пару десятков километров отсюда, бушевало знойное лето."
    scene bg ext_adductius_7dl at sepia_s
    show mt smile pioneer at sepia_s, left
    show anim_grain
    with dissolve2
    "Где-то там строгая с виду вожатая опять сбежала с летучки пораньше под надуманным предлогом, а сама пошла качаться на качелях под раскидистым дубом."
    scene bg ext_lake_sunset_7dl at sepia_s
    show us grin sport at sepia_s, center
    show anim_grain
    with dissolve2
    "Где-то там мелкая девчонка в неизменной красной футболке сидела в засаде у пруда, чтобы поймать зелёного лягушонка. И лучше бы никому не знать, что она собиралась делать с ним после…"
    scene bg ext_stage_normal_clear_7dl at sepia_s
    show dv angry pioneer at sepia_s, right
    show anim_grain
    with dissolve2
    "Где-то там рыжая заноза отчаянно искала, об кого бы ей почесать кулаки."
    "Старые знакомые, которые через каких-то пару часов станут новыми. Олька наверняка уже проинструктировала их на предмет обращения с больным амнезией товарищем."
    show blink
    pause(1)
    scene black
    show bg int_hospital_corridor_7dl
    show unblink
    "А я же всё ещё сверлил глазами больничный плакат, зазубренный до последней чёрточки."
    "Без Виолы в центре явно не справлялись: мой приём традиционно задерживался."
    show blink
    "Я закрыл глаза, чтобы не видеть осточертевший плакат."
    pause(3)
    scene anim prolog_2
    with fade
    $ set_mode_nvl()
    "Мальчик, родившийся с золотой ложкой во рту. {w}Мажор в импортных джинсах. {w}Везунчик, живущий на тёплых островах."
    "Да вот только золотая ложка, импортные джинсы и тропический климат оказались бессильны против неизлечимой болезни."
    "Наверное, судьбе просто нравилось смеяться надо мной."
    "Чужак в другой стране, чужак на родине. Была какая-то горькая ирония в том, что в итоге я стал чужаком даже в собственной голове."
    "Ведь они все называли меня своей шизофренией. Все эти парни, что забирали у меня последнее – меня самого."
    "И, будто этого было мало, они начали забирать у меня и её."
    $ alt_pause(1.0)
    show mt smile dress2 at fright:
        alpha 0.3
    with dissolve2
    $ alt_pause(1.0)
    nvl clear
    nvl show(dissolve)
    "Мне было сложно заглянуть в голову Ольки."
    "Правда ли она верила в те сказки про амнезию, что рассказывала своим подопечным?"
    "Я ведь мог легко прекратить это."
    "Мог бы просто сказать ей, что молния не бьёт в одно место дважды, и каждое новое подселение приносит нового гостя."
    "Но я молчал, а она…"
    "Она никогда не спрашивала."
    "Она сознательно выбрала оставаться в неведении, а я – не развеивать её иллюзий."
    "Потому что всё ещё любил её, даже после того, как самолично всё разрушил."
    nvl hide(dissolve)
    $ alt_pause(0.5)
    ...
    "{i}Как ты мог?!{/i}"
    "{i}У меня не хватает словарного запаса, чтобы выразить всё своё возмущение. Я доверяла тебе, Семён!{/i}"
    "{i}Надеюсь, ты доволен собой. Тебе удалось обмануть меня, глупую девчонку, поверившую в твои небылицы.{/i}"
    "{i}Если тебе интересно, я чувствую себя оплёванной. Мне хочется отмыться от этой грязи.{/i}"
    "{i}Я предала человека, которого люблю. Предала его из-за тебя.{/i}"
    "{i}И от этого так тошно, что я не могу смотреть в зеркало без ненависти к самой себе.{/i}"
    "{i}Посмейся над этим письмом всласть – так же как посмеялся надо мной.{/i}"
    "{i}Надеюсь, ты никогда не узнаешь, что это такое – целовать любимого человека, а потом узнать, что на его месте был кто-то другой.{/i}"
    "{i}Всего хорошего.{/i}"
    "{i}О.Д.{/i}"
    nvl hide(dissolve)
    ...
    $ set_mode_adv()
    $ meet('voice1','Мужик')
    $ renpy.music.play(music_7dl["shehasgone"], channel='music', loop=True, fadeout=3, fadein=3)
    voice1 "Чего смурной такой сегодня?"
    "Из мыслей меня выдернул мужик, чьего имени я никак не мог запомнить. Или просто не хотел."
    am "Сами будто не знаете."
    "Безразлично бросил я, нервно поглядывая на дверь главврача. Его болтовню я на дух не переносил."
    "Он был редким пациентом – своего рода уникумом. Получил свой синдром ПСР в сорок с хвостиком, вопреки врачебной статистике."
    "И шансов прожить долгую и счастливую жизнь имел побольше, чем я и подобные мне."
    voice1 "Ты везучий, парень. Поверь мне на слово – я тут редко пациентов твоих лет больше года наблюдаю."
    voice1 "С нашей болячкой всякое бывает. Прилетит особо приставучий гость, и поминай как звали."
    voice1 "Медсестричка моя говорила, что случай был один лет десять назад…"
    "Эту историю я слышал от пациентов и персонала не раз, но деваться мне было некуда."
    voice1 "Попала сюда девчушка, крохотная совсем, дошколёнок или первоклашка. Синдром получила в лагере, не поделила что-то с другими октябрятами, сбежала в лес, и привет."
    voice1 "Семья у неё была хорошая, благополучная. Люди интеллигентные, не люмпены какие."
    voice1 "Да вот только не повезло девчушке: гостья сильно старше оказалась. Убила её и место заняла окончательно."
    voice1 "А интеллигенция эта вшивая возьми и брось её прямо здесь. Не хотели, дескать, чужого ребёнка растить."
    voice1 "Уж не знаю, что с ней сталось в итоге – в детдом отдали или забрал кто. Но случай показательный."
    "Он выразительно замолчал, по всей видимости ожидая моей реакции. Я лишь пожал плечами."
    am "Всякое бывает. Насмотрелся в этих стенах."
    "Мужик тяжело вздохнул, глядя на меня с показным сочувствием."
    voice1 "Тебя-то твои ни за какие коврижки бы не бросили. Возят, вон, дважды в год с Таити…"
    am "С Кубы."
    "На автопилоте поправил я. От лицемерия моего собеседника хотелось скривиться, но за три года лечения контроль эмоций стал моей сильнейшей стороной."
    "Да и ткнуть меня носом в моё привилегированное положение пытался каждый третий. Не на того напали."
    play sound sfx_open_cabinet_1
    "Дверь в кабинет наконец распахнулась, и я с облегчением подскочил на ноги."
    $ meet('voice','Врач')
    voice "Персунов Семён здесь?"
    scene bg int_ward_day_7dl with dissolve
    "Осмотр был недолгим. Удовлетворённо покачав головой, главврач сделал пометки в медкарте и отпустил меня восвояси."
    scene bg ext_hospital2_away_day_7dl with dissolve
    "Тем более, за углом ЦСР уже поджидал мой автобус."
    "В который раз я задумался над тем, кто же договорился о том, чтобы гонять ради одного меня целый автобус: родители или Виола?"
    "На крыльце меня встретила Верка. Видок у неё был так себе."
    ve "Отстрелялся?"
    "Без лишних приветствий бросила она."
    am "Угу. Случилось что?"
    "Она поморщилась и с силой потёрла виски."
    ve "Мигрень. Если всё готово, то запрыгивай и поехали уже!"
    "Перечить сопровождающей не было смысла."
    th "Раньше начнём – раньше закончим, не так ли?"
    stop music fadeout 3
    show bg int_bus with fade3
    $ renpy.music.set_volume(0.2, delay=0, channel='sound_loop')
    play sound_loop sfx_bus_interior_moving fadein 4
    "Я занял излюбленное место у окна, Верка расположилась неподалёку, опустив на свободное сидение чемодан с медикаментами и сумку для сбора устройств с гостя."
    "С каждым годом она оставляла гостю всё больше. И дело было не столько в рекомендациях Виолы, сколько в отсутствии новизны."
    "Смартфоны, плееры, электронные книги… Едва ли мои копии в других мирах могли утянуть за собой что-то вроде карманного ядерного реактора."
    "А все их безделушки, по словам лечащего врача, снижали их стресс на время пребывания в моём теле.{w} Милым дяденькам в штатском, чьи должности и без формы считывались по выражению лиц, куда проще было стрясти всё наследие иной эпохи с меня после возвращения в город."
    with fade
    play music music_7dl["laugh_throught_the_universe"] fadein 3
    "Голова привычно закружилась, стоило нам выехать за черту города. До подселения оставалось чуть больше получаса."
    "Я вытащил из заднего кармана письмо, которое перечитывал раз десять за это утро. У меня уже не было времени его переписывать, но текст письма не давал мне покоя."
    play sound sfx_7dl["page_turning"]
    $ set_mode_nvl()
    nvl show(dissolve)
    "{i}Мама, папа,{/i}"
    "{i}Это письмо от меня станет последним.{/i}"
    "{i}Я точно знаю, что мой потолок – ещё три подселения, не больше. И не тешу себя иллюзиями, что за полтора года врачи сумеют найти лекарство от моего недуга.{/i}"
    "{i}Если вы читаете это, то знайте: я решил отдать своё тело гостю. У меня была почти неделя на то, чтобы узнать его, и я понял, что он достойный человек.{/i}"
    "{i}Он не принесёт нашей семье лишних неприятностей. Он сумеет начать свою новую жизнь с чистого листа, и едва ли побеспокоит вас больше необходимого.{/i}"
    "{i}Простите, что говорю об этом так. В лицо бы не смог, да и вам было бы куда больнее услышать это, а не прочитать.{/i}"
    "{i}Виола позаботится о том, чтобы его не побеспокоили из ЦСР, поэтому прошу вас лишь об одном: не рассказывайте ему правду.{/i}"
    "{i}Пусть ему не придётся корить себя за украденную жизнь. Я слишком устал от жалости, чтобы моё собственное тело продолжало жалеть меня после того, как я исчезну.{/i}"
    "{i}Люблю вас.{/i}"
    "{i}Семён.{/i}"
    nvl hide(dissolve)
    $ alt_pause(0.5)
    $ set_mode_adv()
    "Наверное, это было не лучшее, что я мог сказать своим родителям напоследок. Но сколько я ни бился над этим письмом, иначе не выходило."
    "Мы стали слишком далеки друг от друга за время моей болезни. А если говорить честнее – мы никогда и не были близки."
    "Я вздохнул и сложил письмо обратно в конверт."
    am "Верка!"
    "Сотрудница ЦСР с тихим стоном открыла глаза."
    ve "Что, уже?"
    am "Нет. Отдай это Виоле, когда пойдёшь отчитываться о моём прибытии."
    scene bg int_bus with dissolve
    "Забрав у меня письмо, Верка вновь уткнулась лбом в стекло, точно это могло облегчить её головную боль. Мы продолжили дорогу до лагеря в тишине."
    scene bg int_bus:
        subpixel True
        xpos 0.5 ypos 1.0 xanchor 0.5 yanchor 1.0 zoom 1.02
        alignaround (.5, .5)
        linear 3.0 yalign 1.0 clockwise circles 1
        repeat
    with dissolve
    "Головокружение усиливалось."
    th "Что я буду делать, если гость снова окажется кретином?"
    th "Я ведь не в том положении, чтобы выбирать. Знал бы, как оно всё обернётся – уступил бы тело самому первому…"
    th "Но задним числом легко быть умным. Тогда и ситуация была другая, да и я сам был совсем другим."
    show blackout_exh with dissolve
    "В глазах потемнело."
    am "Верка…"
    "Хрипло прошептал я. Понял – началось."
    "Девушка принялась рыться в аптечке, доставая шприц и ампулу со снотворным. Я молился всем богам, чтобы из-за недомогания она не напутала с дозировкой: проспать в душном автобусе до самой ночи не хотелось."
    "Картинки в голове становились всё ярче."
    ...
    show prologue_dream
    with flash
    "Зимний парк, утопающий в снегу. Совершенно безлюдный."
    ...
    show prologue_dream
    with flash
    "Магазин, посреди которого стоит мужчина с пистолетом. Лежащий на полу охранник – дедок лет шестидесяти."
    play sound sfx_shoulder_dive_water
    scene bg int_bus_flood_7dl:
        subpixel True
        xpos 0.5 ypos 1.0 xanchor 0.5 yanchor 1.0 zoom 1.02
    ...
    show prologue_dream
    with flash
    "Автобус, тонущий в реке. С десяток размытых лиц, перекошенных от ужаса."
    stop sound_loop fadeout 3
    scene black
    with fade3
    "Плечом я почувствовал холод от спирта, но это всё было так далеко, словно происходило в другой реальности."
    $ renpy.show("uvao_d1", at_list = [left], what = Notch("uvao_d1"))
    show prologue_dream
    with fade3
    "В этой же реальности я видел перед собой лишь два ярко-жёлтых глаза."
    show alt_credits "Ты пойдёшь со мной?" with dissolve2:
        pos (747,105)
    pause(3)
    menu:
        "Пойду, милая":
            $ alt_day_catapult = True
            $ renpy.show("bg int_bus", what = Notch("bg int_bus"))
            show blackout_exh
            with flash_red
            "Игла вонзилась в плечо, но я этого даже не заметил."
            "Меня привычно оттеснило куда-то назад, на задворки собственного сознания."
            scene black with dissolve2
            show alt_credits "Мир снова погрузился в темноту." with dissolve:
                pos (490, 490)
        ...
        "Нет, я останусь здесь":
            ...
            play music "<to 133.1>" + music_7dl["intro"] fadein 5
            $ renpy.show("bg int_bus", what = Desat("bg int_bus"))
            with dissolve
            show alt_credits "Наваждение спало так же быстро, как и началось.":
                pos (280, 790)
            with dissolve
            $ renpy.pause(7.0, hard=True)
            hide alt_credits with dspq
            show alt_credits "Я вновь видел перед собой лишь автобус, проносящиеся за окном пейзажи и хмурую Верку со шприцем в руке.":
                pos (70, 890)
            with dissolve
            $ renpy.pause(7.0, hard=True)
            hide alt_credits
            ...
            show blackout_exh
            show alt_credits "Всё было так, словно ПСР только что отчалил, предоставив меня самому себе. С той лишь разницей, что в этот раз он даже не посетил меня.":
                pos (10, 890)

            show alt_credits "Плечо пронзила острая боль, и я откинулся на спинку сидения, закрывая глаза. Странностей становилось всё больше, а голова становилась всё тяжелее…":
                pos (70, 890)
            with dissolve2
            $ renpy.pause(4.8, hard=True)
            show alt_credits "Отключался я с отчётливой мыслью:" with dissolve:
                pos (480, 390)
            $ renpy.pause(2.2, hard=True)
            scene black
            hide alt_credits
            with dissolve2
            show alt_credits "В этот раз никто не придёт." with dissolve:
                pos (590, 490)
