label alt_day2_begin:
    ...
    th "Порошок! Я уже очень давно не видел ничего подобного."
...
label alt_day2_bf:
    ...
    menu:
        ...
        "Сесть с Леной":
            ...
            "Я не стал набирать много, ограничился парой бутербродов и чашкой с мерзким какао. Додумались ведь разбавить хороший напиток сгущёнкой!"
    ...
    menu:
        ...
        "Алиса":
            ...
            if lp_dv >= 3:
                ...
                dv "Ладно. Стой."
                show dv normal pioneer2 with dspr
                dv "Поможешь мне кое с чем, потом дела. Годится?"
...
label alt_day2_sl_guide:
    ...
    me "Да и вообще, тебе больше всех надо?"
    "Славя не отреагировала на моё ворчание."
...
label alt_day2_event_music_club1:
    ...
    if (alt_day2_convoy == 'dv_prep'):
        ...
        if herc:
            ...
            dv "Ты куда?"
            me "Я сейчас!"
            scene bg ext_house_of_mt_day with touch
            $ alt_pause(1.5)
...
label alt_day2_inmusic:
    ...
    if ('music' in list_joined_clubs_7dl):
        ...
        mi "Но Ольга Дмитриевна велела мне сидеть в клубе и ждать новичка — и чтобы обязательно записала его в кружок!"
        if 'music' in list_joined_clubs_7dl:
...
label alt_day2_event_clubs1:
    ...
    if (alt_day2_convoy == 'un'):
        ...
        show sl smile2 pioneer with dspr
        if ('library' in list_voyage_7dl):
            sl "Похоже, что Шурик всё ещё в библиотеке."
            me "А при чём здесь Шурик?"
            show sl smile2 pioneer with dspr
    ...
    if (alt_day2_convoy == 'sl'):
        ...
        sh "Это не займёт много времени."
...
label alt_day2_event_library1:
    ...
    if (alt_day2_convoy == 'un'):
        ...
        me "Зачем?"
        dv "Да кто ж их знает. Может, в карты играют."
...
label alt_day2_event_medic_house1:
    ...
    if (alt_day2_convoy == 'un'):
        ...
        show dv normal pioneer far at fright with dissolve
    ...
    "В горле пересохло, и я проглотил окончание слова. Из-за стола в дальней части кабинета встала крайне фигуристая разноцветноглазая девица."
    ...
    cs "…лечение."
    if (alt_day2_convoy == 'un'):
        show un shy pioneer far at fright with dspr
    elif (alt_day2_convoy == 'sl'):
        show sl shy pioneer far at fright with dspr
    elif (alt_day2_convoy == 'dv'):
        show dv shy pioneer far at fright with dspr
    me "Да я здо…"
    cs "Это ты зря."
    ...
    if (alt_day2_convoy == 'un'):
        show un laugh pioneer far at fright with dspr
        ...
        show sl tender pioneer far at fright with dspr
        ...
        show dv surprise pioneer far at fright with dspr
        ...
        show dv shocked pioneer far at fright with dspr
        dv "ЧТО?!"
...
label alt_day2_dubstep2:
    ...
    if (alt_day2_convoy == 'un'):
        ...
        $ lp_mi += 1
        show mi happy pioneer far at fright with dissolve
    ...
    $ renpy.music.play(music_7dl["splin"], channel='music', loop=True, fadeout=3, fadein=3)
    ...
    if (alt_day2_convoy == 'sl'):
        ...
        show dv normal pioneer2 far at cleft behind us with dissolve
        $ alt_pause(0.5)
        show dv normal pioneer2 at cleft behind us with dspr
        show sl smile2 pioneer at fright with dspr
        ...
        show dv normal pioneer2 at cleft behind us with moveinleft
    ...
    if alt_day2_convoy != 'dv':
        show dv smile pioneer2 at cleft behind us with dspr
    ...
    if (alt_day2_convoy == 'sl') and alt_day1_us_shotted and ('medic' in list_voyage_7dl):
        ...
        show dv sad pioneer2 behind us at cleft with dspr
    ...
    menu:
        "Ключи у меня!":
            ...
            if not ('music_club' in list_voyage_7dl):
                sl "Давай в первую очередь пойдём к Мику?"
            else:
                sl "Теперь можно зайти в библиотеку или медпункт."
...
label alt_day2_event_boat_station1:
    ...
    if (alt_day2_convoy not in ('dv', 'sl', 'un')):
        ...
        if ('music' in list_joined_clubs_7dl):
            ...
            "Окинув взглядом пришвартованные судёнышки, я убедился, что они довольно небрежно пришвартованы к причалу, а вёсла никто не удосужился снять."
            "Любой желающий может взять лодку и устроить небольшое рандеву или вовсе совершить побег."
            "Я представил, как запрыгиваю в лодку и под сопровождение из криков лодочника и апплодисментов пионеров отправляюсь в приключение."
            dreamgirl "Ты уже вляпался в приключение покруче прочих, а вместо того, чтобы наслаждаться им сполна, сидишь и думаешь о ерунде."
            th "Зануда."
            "Услышав шаги позади, я обернулся."
            "Мимо меня промчался какой-то лохматый пионер, примерно ровесник Ульяны."
            th "И куда он так торопится?"
            "Пионер напомнил о том, что меня тоже ждут дела, а потому я встал с понтона и отправился дальше."
    else:
        if (alt_day2_convoy == 'dv'):
            show dv normal pioneer2 with dissolve
            dv "Ну и зачем мы сюда припёрлись?"
            me "Как это зачем? Уточек кормить!"
            show dv surprise pioneer2 with dspr
            "Двачевская посмотрела на меня, как на прокажённого."
            dv "Ты это серьёзно сейчас?"
            me "Совершенно серьёзно."
            "Мне стоило огромных усилий не заржать, глядя на то, как девочка хмурится в недоумении."
            me "Но все уточки, кажется, улетели на юг…"
            show dv angry pioneer2 with dspr
            dv "Да иди ты!"
            me "А я и иду. Ты со мной?"
            hide dv with dissolve
            "Процедив сквозь зубы что-то непечатное, Алиса обогнула меня и первая направилась прочь с пристани."
        elif (alt_day2_convoy == 'un'):
            show un normal pioneer far with dissolve
            "Лена молча стояла чуть поодаль от меня, то и дело поглядывая на домик у пристани."
            me "Там заведующий лодками обитает?"
            "Предположил я."
            "Ответом мне послужил лишь короткий кивок."
            th "Вот и поговорили…"
            me "Ладно, подпись он мне в любом случае не поставит. Пойдём дальше."
            "Лена снова кивнула, и я развернулся в сторону лагеря."
        else:
            show sl smile pioneer with dissolve
            sl "Мне здесь тоже нравится."
            "Бросила Славя, точно прочитав мои мысли о пристани."
            sl "Видишь те острова вдалеке?"
            "Я проследил за направлением её руки и утвердительно кивнул."
            show sl smile2 pioneer with dspr
            sl "На самом деле, до них плыть на лодке всего ничего. Знал бы ты, какая сладкая земляника там растёт!"
            me "Предлагаешь прямо сейчас туда смотаться?"
            th "Ради земляники я бы, конечно, так напрягаться не стал, но вот ради того, чтобы отплатить Славе за всю её помощь…"
            show sl laugh pioneer with dspr
            sl "Тебе бы для начала с обходным закончить!"
            th "Ну и ладно. Не сильно-то и хотелось!"
            show sl smile2 pioneer with dspr
            sl "Так что давай немного ускоримся. Обед уже не за горами, а мы ещё толком нигде и не побывали!"
    stop music fadeout 3
    stop ambience fadeout 6
    with fade
    return
...
label alt_day2_dinner:
    ...
    play ambience ambience_dining_hall_full fadein 3
    "Кажется, сегодня пионеры были голоднее обычного. Во всяком случае, свободных мест было здорово меньше, чем на завтраке или даже вчерашнем ужине. Том самом, с малазийской кухней."
    ...
    if 'music_club' in list_voyage_7dl:
        $ miku_menu = "С Мику"
    if ((not alt_day1_us_shotted) and (alt_day2_convoy not in ('dv_prep', 'dv_rej'))) or (alt_day2_convoy == 'dv'):
        menu:
            "[miku_menu]":
                label alt_day2_mi_dinner:
    ...
    us "Легко!"
    "Она протянула руку, схватила стакан покрепче, дёрнула, и…"
    ...
    scene
    $ renpy.show("bg ext_warehouse_day_7dl", at_list=[fast_running], what = Noon("bg ext_warehouse_day_7dl"))
    with dspr
...
label alt_day2_grand_escape:
    ...
    "Мелкая скрылась внутри, а меня вдруг осенило — я узнал этот взгляд!"
    "Взгляд этот может означать только одно — {i}женщина изволит сходить с ума, вы приглашены, явка обязательна{/i}."
    show us normal sport bear with dissolve
    "Снова скрипнула дверь, и на пороге показалась Ульяна — она сменила обувь на кеды и к груди прижимала кулёк с конфетами и медведя."
    ...
    us "И долго нам ждать этот поезд?"
    me "Откуда мне знать? Расписанием не владею, увы!"
    "Ядовито отозвался я."
    us "И что, нам теперь весь день тут торчать?"
    me "Если придётся. А может и ночевать."
    show us dontlike sport with dspr
    us "Какой-то дурацкий у тебя план!"
    "Ульяна состроила недовольную гримаску и начала ныть."
    us "Нашу пропажу заметят, точно тебе говорю. А потом найдут, а потом накажут!"
    us "Зачем я тебя послушала? Надо было через лес…"
    "Я демонстративно зевнул."
    me "Кончай уже хныкать, это изначально была твоя идея."
    ...
    th "В конце концов, должен же я поиметь хотя бы немного профита с этого дурацкого путешествия?"
    with fade3
    "До отвала наевшись земляники, я блаженно растянулся под деревом."
    "От усталости и жары меня начало клонить в сон."
    show us dontlike sport with dspr
    us "И чего ты тут разлёгся? А кто поезд сторожить будет?"
    me "Тебе надо – ты и сторожи. А мне и тут неплохо."
    ...
    play sound sfx_heavy_breathing
    ...
    show blink
    "Я присел под деревом и прикрыл глаза."
    play music music_list["orchid"] fadein 7
    "И вдруг услышал прямо над собой голос Ольги Дмитриевны:"
    ...
    pause(3)
    th "Какого чёрта? Когда они успели к нам подобраться?"
    dreamgirl "Большое дело — поймать двух спящих пионеров."
    th "Да я же только глаза прикрыл!"
    dreamgirl "Угу, уже три часа прошло."
    th "Бред! Поезд ведь только что…"
    dreamgirl "А, ты про это кино?"
    th "Кино? Это был сон?"
    dreamgirl "Угу. Но это не самое забавное открытие этого вечера."
    "Я вдруг понял, что сплю не один. Вернее сказать, сплю не совсем сам по себе."
    ...
    "Это заставило меня проснуться окончательно."
    me "Аааааа!"
    ...
    us "Ты чего орёшь? Что случилось?"
    ...
    $ alt_day3_duty = True
    show us sad sport bear at left with dissolve
    us "Ну мы ведь только на остров скатались. И немного ягод поели, честно-честно."
    mt "Я всё сказала! Дежурство в столовой!"
    ...
    dv "А когда Ольга Дмитриевна сказала, что ты исчез вместе с ней, то я сразу всё поняла."
    ...
    dv "На самом деле это была проверка."
    "Я не поверил ушам. То есть, весь этот план побега был понарошку?"
    me "И что это за проверка?"
    dv "Это Улькина идея. Всё хотела узнать, хватит ли у тебя духу нарушить правила и сбежать."
    dv "Надеюсь, ты не будешь на неё злиться?"
    "Я призадумался и не ощутил в себе никакой обиды."
    me "Не буду. Если подумать, меня никто за руку не тащил, верно?"
    "Алиса понимающе кивнула."
    me "А ты очень сильно за неё переживаешь."
    dv "А как иначе? По секрету скажу, ты только никому не говори…"
...
label alt_day2_siesta:
    ...
    "Например, вожатая. Ей совсем до лампочки, что у неё в отряде из ниоткуда появился пионэр?"
    ...
    if (alt_day2_convoy == 'un'):
        ...
        play music music_7dl["ambience_safe"] fadein 4
...
label alt_day2_tournament:
    ...
    if not ('library' in list_voyage_7dl):
        ...
        "Я решил, что следует подойти и познакомиться с последним представителем здешнего бестиария, но Ульяна схватила меня под локоть."
        me "В чём дело?"
    ...
    if not alt_day2_mi_met:
        ...
        hide mi with dissolve
    if not ('sport_area' in list_voyage_7dl):
        show ba normal uniform far at fleft
        show us smile pioneer at right
        with dissolve
        us "Дядя Боря!"
        "Радостно замахала вдруг Ульяна здоровому мужику в спортивной форме."
        th "А это ещё что за гусь?"
        "Помнится, я видел его в столовой. Или не видел?"
        me "Ульяна, а что это за дядька?"
        "Шёпотом спросил я."
        show us calml pioneer at right with dspr
        us "Дядя Боря это, не слышал, что ли?"
        "Я закатил глаза."
        me "А можно чуть больше деталей?"
        show us normal pioneer at right with dspr
        us "Борис Александрович, наш физрук. Наверное, пришёл фотографировать турнир!"
        $ meet('ba','Саныч')
        "Действительно, на шее у физрука болтался громоздкий фотоаппарат."
        "Видок у него был довольно скучающий. Наверняка его притащили сюда против его желания."
        "Хотя я искренне не понимал, как можно заставить такого человека делать что-то против его воли — руки этого человека наверняка способны сгибать даже толстые металлические балки."
        hide ba
        hide us
        with dissolve
...
label alt_day2_walk_sl:
    ...
    if not herc:
        ...
        me "Потому что… Трудно объяснить в нескольких словах. Давай просто скажем…{w} Что люди сделали всё от них зависящее, чтобы утратить остатки моего доверия."
...
label alt_day2_ultim:
    ...
    dv "…лапал!"
    th "Интересно, а сидящая в кустах Ульянка против кого свидетельствовать будет, если дело дойдёт до суда?"
    th "С одной стороны подруга, а с другой — справедливость…"
    dreamgirl "Я бы не надеялся на её лояльность."
    ...
    if loki or herc:
        ...
        menu:
            "Поспорить":
                ...
                show un normal pioneer at right
                with dissolve
                "Я уже было собирался ответить, как у меня из-за спины бесшумно вынырнула Лена и бросила какой-то странный взгляд на Алису."
                show dv angry pioneer2 at center with dspr
                dv "Чего тебе?"
                show un scared pioneer at right with dspr
                un "Н-ничего…"
                hide un with dissolve
                "Бедная девочка опустила голову и поспешила скрыться в столовой."
                me "Лады."
            ...
            "Не спорить":
                ...
                "Стараясь глядеть в пол, Лена попыталась просочиться мимо нас."
                "Но делала это так медленно, что Алиса нетерпеливо цокнула языком."
                ...
                if (alt_day2_convoy == 'un'):
                    ...
                    dv "Это я ещё ласково. Ты споришь или нет?"
                me "Не буду я с тобой спорить."
                "Из кустов послышался разочарованный стон."
                "Я вежливо отодвинул Алису в сторону и зашёл в столовую вслед за Леной."
            ...
            "Чтобы было честно":
                ...
                dreamgirl "Я тоже хотел бы знать."
                show dv surprise pioneer2 far at center
                show dv angry pioneer2 far at center
                with dspr
                "Алиса дала мне по рукам и отскочила в сторону."
                ...
                "А для этой пацанки такие материи были слишком тёмными."
                me "Ладно. Можешь теперь считать, что мы поспорили."
                "Пожалел я её."
                me "Хотя за тобой ещё переодевание."
                "Девочка бросила ещё один злобный взгляд и, толкнув меня плечом, залетела в столовую."
                dreamgirl "Как бы тебе это боком не вышло."
                th "Да брось, кому она расскажет? {w}Да и кто ей поверит?"
                "Решив не медлить, я поспешил в столовую и чуть не столкнулся с Леной лбами."
                un "Ой!"
                "Выглядела девочка так, словно хотела провалиться сквозь землю."
                mt "Семён! Лена! Вы чего там застряли в дверях? {w}Бегом сюда!"
                "Я махнул рукой и кивнул, а Лена втянула голову в плечи."
                "Видимо, своим появлением я сорвал планы Лены на незаметный побег."
                "Так что ей пришлось идти на отрядное мероприятие, пристроившись в моём кильватере."
    else:
        $ alt_day2_dv_ultim = 1
...
label alt_day2_supper:
    ...
    $ renpy.music.set_volume(0.9, delay=2, channel='sound')
    play sound sfx_concert_applause fadein 1
    ...
    if alt_day2_gamblers_result['mi'] >= 21:
        "Мику же, которая на несчастное кондитерское изделие прав имела никак не меньше, держалась поодаль."
    ...
    if (counter_sl_7dl == 3) or (alt_day2_dv_ultim == 2):
...
label alt_day2_sup2:
    ...
    if alt_day2_dv_ultim == 2:
        ...
        show dv angry pioneer2 with dspr
        "Алиса прищурилась."
        dv "Нет свидетелей, говоришь? Ульянку не послушают?"
        "Я кивал после каждого вопроса."
        dv "А Лену послушают, как думаешь?"
        "Улыбка сползла с моего лица."
        th "Блин! Лена всё видела! Как я мог забыть?"
        me "Не думаю, что ей есть дело до наших разборок…"
        "Не очень-то уверенно протянул я."
        show dv grin pioneer2 with dspr
        dv "Интересно, а о чём тогда там Лена с физруком разговаривала, тыкая в тебя пальцем сразу после ужина?"
        "Я похолодел."
        show dv rage pioneer2 close with dspr
        "Алиса грозно улыбнулась и приблизилась ко мне вплотную."
        dv "Сильно сомневаюсь, что она обсуждала с ним график тренировок по бадминтону."
        show dv smile pioneer2 with dspr
        dv "Хочешь ещё одно пари?"
        me "Какое?"
        dv "Сможешь ли ты убежать от нашего физрука?"
        me "Может я просто смогу ему всё объяснить?"
        show dv grin pioneer2 with dspr
        "Двачевская посмотрела на меня, как на идиота."
        dv "Можешь попытаться. Но раз мы с тобой сегодня стали очень близки…"
        show dv rage pioneer2 with dspr
        "Она снова злорадно улыбнулась."
        dv "…то я дам тебе один совет — лучше беги!"
        dv "Потом спасибо скажешь. {w}Если сможешь говорить."
        hide dv with dissolve
        "Она хлопнула меня по плечу и с довольной улыбкой зашагала по своим делам."
        th "Стерва!"
        "И это я не про Алису!"
...
label alt_day2_run:
    ...
    scene bg ext_dining_hall_near_sunset with fade
    "Я стоял и судорожно размышлял, как мне выйти из этой ситуации."
    play music music_7dl["anglegrinder"] fadein 1.5
    "А между тем, она всё ухудшалась и ухудшалась."
    show mt normal pioneer with dissolve
    "За спиной скрипнула дверь, и на крыльцо вышла Ольга Дмитриевна."
    show ba em1 uniform behind mt at cright with dissolve
    extend " А за её спиной возвышался ОН…"
    th "Ну и что мне делать?"
    scene bg ext_houses_sunset at running with CropMove(0.4, mode="slideawayright")
    "Вожатая открыла рот, чтобы что-то сказать, но я уже выполнил маневр «Кру-гом!» и дал дёру!"
    mt "Семён! Ты куда?"
    "Она ещё что-то кричала мне вслед, а через секунду к её голосу присоединился звучный бас, только ускоряя движение моих ног."
    scene bg ext_path2_sunset_7dl with CropMove(0.4, mode="slideawayright")
    play sound sfx_7dl["breath"]
    "Я одним махом проскочил площадь, минуя пионеров помладше, испуганно отскакиващих с моего пути, и остановился за стеной здания клубов."
    me "Вот… {w}дерь… {w}мо…"
    play sound_loop sfx_7dl["heartbeat"]
    "Я судорожно хватал ртом воздух, пытаясь отдышаться, а сердце колотилось как бешеное."
    stop sound
    play sound sfx_bush_leaves
    "И треск в кустах, раздавшийся через пару секунд, чуть не стал причиной моего инфаркта."
    stop sound_loop
    show us laugh2 sport with dissolve
    play music music_7dl["runaway"] fadein 3
    us "Здарова, новичок!"
    "Я тяжело выдохнул."
    me "А. Это ты."
    "Мелкая, видимо, уже успела прикончить торт и умыться — на лице и руках не было следов крема."
    show us smile sport with dspr
    us "Чего делаешь?"
    "Я развёл руки в стороны."
    me "А на что похоже?"
    show us grin sport with dspr
    us "Похоже на то, что я не единственная, кого хочет убить вожатая!"
    "Я кивнул."
    show us normal sport with dspr
    us "Видела я, как ты от них улепётываешь."
    show us laugh sport with dspr
    "Она улыбнулась."
    us "Тебе не говорили, что ты очень смешно бегаешь?"
    me "Да иди ты!"
    show us upset sport with dspr
    "Ульяна фыркнула."
    us "Фу, как грубо! {w}Надо быть вежливее с товарищами!"
    me "Товарищами?"
    show us laugh2 sport with dspr
    us "Ну да. Вот сейчас мы с тобой оба попали в опалы к вожатой."
    me "Опалу."
    "На автомате поправил я."
    show us smile sport with dspr
    us "Да, точно! Мы теперь товарищи по несчастью!"
    if alt_day2_us_escape:
        me "Мы уже, забыла? Нам завтра дежурить!"
        show us surp1 sport with dspr
        us "Ой, правда ведь! Тем более!"
    show us laugh sport with dspr
    "Она хлопнула меня по плечу."
    scene bg ext_clubs_sunset_7dl with dissolve
    us "Ну и чего ты натворил?"
    "Я не спешил с ответом, выглядывая из-за стены, чтобы убедиться в отсутствии опасности поблизости."
    "Пока что всё было относительно спокойно: ни Ольги Дмитриевны, ни Саныча видно не было."
    us "Эй! Чего молчишь?"
    scene bg ext_path2_sunset_7dl
    show us sad sport
    with dissolve
    "Я обернулся."
    me "Я…"
    th "А стоит ли мне вообще рассказывать?"
    dreamgirl "На твоём месте я бы лучше помалкивал."
    th "В другой ситуации я бы так и сделал. Но вдруг она подскажет, что можно сделать?"
    th "Учитывая то, как часто она попадает в неприятности, ключики к сердцам местных воспитателей давно подобраны, раз она до сих пор в лагере."
    me "Ну… {w}Ты же видела, там, у столовой, как мы с Алисой…"
    show us normal sport with dspr
    us "Спорили?"
    me "Да. Так вот, ты же видела, как я…"
    show us laugh2 sport with dspr
    us "Ну, поспорили, а что такого?"
    "Я возвёл глаза к небу."
    th "Какая же она непонятливая!"
    me "Как я потрогал Алису за грудь!"
    "Выпалил я."
    show us fear sport:
        linear 1.2 zoom 0.90
    with dspr
    us "Э-э-э???"
    "Ульянка инстинктивно обхватила себя руками и начала медленно пятиться назад."
    show us upset sport:
        zoom 0.90
    with dspr
    us "А зачем ты это сделал?"
    me "Она же… {w}Стоп. {w}Ты же была там, я тебя в кустах видел!"
    show us surp3 sport:
        zoom 0.90
    with dspr
    us "Я… я была, да. {w}Но я слышала только про спор, а потом пригнулась, чтобы Лена меня не увидела."
    us "А когда поднялась, Алиса уже залетела в столовую!"
    "Я заскрипел зубами."
    show us surp2 sport:
        linear 1.0 zoom 0.85
    with dspr
    "Ульянка продолжала пятиться, не отрывая от меня своего взгляда."
    me "Расслабься ты, не буду я тебя трогать! Я не извращенец какой-то."
    me "Да там и нечего…"
    "Непонятно зачем ляпнул я. Мои слова достигли ушей Ульяны…"
    show us angry sport:
        zoom 0.85
    with dspr
    extend " с ожидаемым эффектом."
    show us angry sport close with dspr
    with vpunch
    "Мелкая накинулась на меня и начала метелить, нанося удары туда, куда доставала своими ручонками."
    us "Ах…"
    with hpunch
    extend " Ты…"
    with vpunch
    ...
    extend " КОЗЁЛ!!!"
    hide us with moveoutright
    "Наконец выпалила вся красная Ульяна и, развернувшись, убежала."
    "Я прислонился к стене."
    th "Ну и что мне теперь делать?"
    dreamgirl "Для начала — научился бы держать язык за зубами. Ты вообще думаешь, когда говоришь?"
    "Я хотел по привычке огрызнуться, но не стал. Внутренний голос был прав."
    "Теперь к Алисе, Лене, вожатой и физруку прибавилась Ульянка. Вот весь список людей, с которыми мне больше не хотелось встречаться."
    mt "Семён! Семён!"
    scene bg ext_clubs_sunset_7dl with dissolve
    $ alt_pause(0.5)
    show mt normal pioneer far
    with dissolve
    "Я выглянул из-за угла: вожатая шла в мою сторону."
    th "Долго здесь я прятаться не смогу!"
    th "Надо что-то делать."
    stop ambience fadeout 2
    stop music fadeout 2
    "В голове появилась картина:"
    play music music_7dl["dropit"] fadein 2
    scene bg ext_square_sunset
    show prologue_dream
    show mt angry pioneer behind prologue_dream
    show ba normal uniform behind mt, prologue_dream at cright
    show un normal pioneer behind prologue_dream at left
    show dv normal pioneer2 behind prologue_dream at fright
    with dissolve2
    mt "Примерный пионер никогда в жизни не тронет грудь девушки без её разрешения!"
    mt "Это недостойное поведение для будущего комсомольца и мужчины в целом!"
    "Я стоял на площади, окружённый людьми."
    "Вокруг меня собрались пионеры, галдящие и тыкающие в меня пальцами."
    "Передо мной стоит Ольга Дмитриевна, чуть позади наш доблестный физрук, а по бокам от них Лена и Алиса."
    mt "Ты лишаешься значка пионера! Сними галстук!"
    show un laugh pioneer
    show dv laugh pioneer2
    with dspr
    $ meet('voice','Толпа')
    "Толпа засмеялась и загалдела."
    voice "Позор! Позор!"
    $ meet('voice','Голос')
    show dv normal pioneer2 close at fright with dspr
    $ alt_pause (0.6)
    show dv smile pioneer2 at fright with dspr
    "Алиса сорвала с меня значок и победно улыбнулась."
    show un normal pioneer close at left with dspr
    "А Лена, развязавшая мой галстук, прошептала мне на ухо:"
    show un snide_smile pioneer close at left with dspr
    if persistent.me_neu_loki_true and persistent.me_neu_dr_true and persistent.me_neu_herc_true:
        un "Надо было соглашаться."
        show un laugh pioneer close at left with dspr
        "И громко засмеялась…"
    else:
        un "Ты сам всё испортил…"
    show un snide_smile pioneer at left with dspr
    stop music fadeout 2
    $ alt_pause (1.0)
    play ambience ambience_camp_center_evening fadein 2
    ...
    with Dissolve(1.5)
    th "Бежать! Срочно!"
    "Дождавшись, когда вожатая отвернётся, я выскочил из-за здания и побежал к воротам."
    stop sound_loop
    scene bg ext_camp_entrance_sunset_7dl with dissolve
    $ alt_pause (0.7)
    scene bg ext_road_sunset at running with dissolve
    "Прошмыгнув за них, вырулил на дорогу и ускорился."
    show dreamgirl_overlay with dissolve
    dreamgirl "Каков план?"
    "Я молчал. Никакого продуманного плана не было."
    th "Главная задача — не попасть в руки огромного медведя, которого в лагере поставили преподавать детям физкультуру."
    dreamgirl "С чего ты решил, что убежать из лагеря будет лучше, чем попробовать объясниться?"
    th "Алиса сказала, что лучше бежать…"
    dreamgirl "А ещё она хотела рассказать всему лагерю о том, что ты её лапал до того, как ты это сделал."
    th "Если проиграю…"
    dreamgirl "Уверен, что стоит принимать её слова всерьёз?"
    th "Да пофигу на неё. Меня же могут поставить на учёт! Или вообще посадить!"
    th "Думаешь, просто так про «аморалку» в Союзе говорили?"
    th "Выгонят из пионеров, напишут характеристику и привет! Все дороги закрыты!"
    dreamgirl "А твой побег что-то изменит?"
    "Я замедлился."
    "И правда, простой побег ведь не решит эту проблему!"
    th "Нужно добыть новые документы!"
    dreamgirl "Ты меня вообще слушаешь?"
    th "Интересно, далеко тут до ближайшего города?"
    dreamgirl "Эй, на палубе! Ты несёшь какой-то бред!"
    th "Там затеряться можно, интернета же нет. Пока меня объявят в розыск, пока развесят объявления…"
    dreamgirl "АЛЛО! ТЫ СОВСЕМ УЖЕ С УМА СОШЁЛ?"
    hide dreamgirl_overlay with dissolve
    th "Заткнись."
    "Внезапно голова прояснилась и в ней созрел план."
    "Доехать на попутке до города, там скинуть форму и разжиться нормальной одеждой. Осталось решить, что из моих вещей проще будет продать: плеер или телефон?"
    "Стало тихо. Я запоздало понял, что это внутренний голос перестал создавать в голове ненужный фон."
    stop ambience fadeout 2
    stop music fadeout 2
    $ alt_pause(1.0)
    play ambience ambience_camp_entrance_night fadein 3
    scene bg ext_road_night2 with fade2
    "А день, тем временем, уже закончился. С поля подул прохладный ветер."
    "Я оглянулся назад и удивился тому, как далеко убежал. Ворота лагеря едва были различимы."
    "И судя по тому, что на дороге не было ни души, и фонари не прорезали темноту вечера, мой уход через ворота остался незамеченным."
    "Облегчённо вздохнув, я сошёл с асфальта и присел возле невысокого деревца. Я сильно устал и мне нужен был отдых."
    "Вытянув ноги и облокотив спину о тонкий ствол, я смотрел в сторону горизонта."
    "Внутри разрасталось странное, уже позабытое, какое-то детское ощущение… свободы."
    "Я улыбался, представляя, как начну новую жизнь. Авантюра чистой воды, да, но как же она меня вдохновляла!"
    if herc:
        "Да и что я теряю? Каким бы ни было это приключение, оно всяко интереснее моей прошлой жизни."
        "Только бы соседи додумались выпустить Маху…"
    else:
        "Это же именно то, что нужно!"
        "Новый лист, новые чернила, новое перо. {w}И новая история!"
    "В груди немного кольнуло — всё-таки за эти неполные два дня я успел немного попривыкнуть к лагерю и его обитателям."
    "Но как там говорится: «Когда закрывается одна дверь, открывается новая»?"
    stop ambience fadeout 2
    "Именно с этой мыслью я и уснул."
    show blink
    $ alt_pause(1.0)
    play ambience ambience_medstation_inside_night fadein 2
    play music music_7dl["comfortable_mistery_3"] fadein 2
    scene bg int_aidpost_night
    show prologue_dream
    with fade2
    $ set_mode_nvl()
    voice "И что мне с ним теперь делать?"
    "Знакомый голос. Это вожатая?"
    voice "Успокойся. Учитывая обстоятельства, он больше не твоя головная боль."
    "Второй голос был грубее, ниже. Явно взрослый и мужской."
    th "Что за…"
    "Голова раскалывалась. Не было сил даже разлепить веки."
    mt "Может поговорить с ним? Я уверена, что всему есть объяснение!"
    "Мужчина вздохнул."
    voice "Поговорим, конечно. {w}Но если честно, будь моя воля — дал бы пинка под зад и отправил бы обратно к мамочке!"
    mt "Нельзя же так, Борис Александрович!"
    ba "Можно! И нужно!"
    if persistent.me_neu_loki_true and persistent.me_neu_dr_true and persistent.me_neu_herc_true:
        ba "Этот дрищ может быть опасен! Он уже домогался до пионерки и после этого сбежал из лагеря!"
        mt "Но он же вернулся! К тому же это Алиса… {w}Она могла спровоцировать его."
        "Физрук ухмыльнулся."
        ba "Послушай-ка, девочка. Ты здесь адвокат его или вожатая?"
        "Холодный тон Бориса Александровича действительно внушал страх."
        mt "В-вожатая…"
        nvl clear
        ba "Что-то не видно. Защищаешь нарушителя, девочку виноватой выставляешь, поселила этого дрища у себя…"
        mt "Н-но…"
        ba "Не «нокай»! Взял бы за шкирку да вытряс бы из него всю дурь — да регламент не позволяет!"
        ba "Так что лучше бы тебе помолчать."
        "Ледяным тоном завершил свою речь физрук."
        ba "Развели тут… {w}Воспитатели хреновы!"
    play sound sfx_table_hit
    play sound2 sfx_open_dooor_campus_1
    "Он ударил кулаком по столу, вожатая испуганно вскрикнула и в ту же секунду скрипнула дверь."
    nvl clear
    "Раздался стук каблучков, а новый голос угадывался безошибочно."
    cs "Борис! Ты не забыл, где ты находишься?"
    ba "А, да. Извини."
    $ set_mode_adv()
    hide prologue_dream
    scene bg int_aidpost_night
    ...
    show unblink
    "Огромным усилием я приоткрыл один глаз и увидел, как физрук ставит на место упавшую лампу."
    show mt sad pioneer at right with dissolve
    "Вожатая сидела, спрятав лицо в ладонях. Её плечи начали мелко вздрагивать."
    show cs normal with dissolve
    cs "Оля, можешь идти."
    "Ольга Дмитриевна подняла взгляд."
    mt "А что будет с ним?"
    "Она кивнула в мою сторону. Виола пожала плечами."
    show cs serious with dspr
    cs "Уже поздно, Оль. Завтра утром будем решать."
    "Она хмыкнула."
    cs "В конце концов, мы даже не услышали, что ему есть нам сказать."
    show mt normal pioneer with dspr
    mt "Хорошо. Спокойной ночи."
    hide mt with dissolve
    "Как только вожатая ушла, Виола обратилась к физруку."
    show cs doubt with dspr
    cs "Как вы его нашли?"
    "Я затаил дыхание. Наконец-то!"
    $ renpy.music.set_volume(0.4, delay=1, channel='music')
    "Всё то время, пока я подслушивал их диалог, у меня в голове вертелась одна только мысль:"
    play sound sfx_7dl["scary_piano"]
    with vpunch
    th "КАКОГО ЧЁРТА Я ДЕЛАЮ В МЕДПУНКТЕ???"
    show ba normal uniform with dspr
    $ renpy.music.set_volume(1.0, delay=1, channel='music')
    ba "Я решил ещё один обход вокруг лагеря совершить, вышел за территорию. {w}А там он…"
    cs "Он был в сознании?"
    show ba em1 uniform with dspr
    "Физрук шумно почесал в затылке."
    ba "Как сказать… Он меня увидел, прохрипел что-то и обмяк. Пришлось тащить его на себе."
    ba "Так что сюда он попал уже без сознания."
    show cs normal with dspr
    cs "Интересненько."
    show blink
    "Она подошла поближе, и я прикрыл глаз."
    "Вся моя выдержка в этот момент работала на то, чтобы не сделать судорожный вдох."
    "Она смотрела на моё лицо, это ощущалось нутром. И от этого становилось… {w}страшно."
    "Я вдруг осознал, насколько беспомощен здесь, лёжа на кушетке, когда надо мной нависает жуткая медсестра и огромный детина."
    "Ещё и этот рассказ физрука…"
    th "У меня начались провалы в памяти? Почему я пришёл обратно?"
    "Под прикрытием тонкого одеяла, сжав кулаки, я ждал. Едва заметно двигал ногами, шевелил пальцами."
    "С каждой минутой я всё лучше чувствовал своё тело."
    cs "Ладно, Борис, иди-ка ты тоже спать."
    show ba smile uniform with dspr
    ba "Может мне с тобой посидеть? Мало ли."
    "Медсестра тихонько засмеялась."
    cs "Ну уж нет! Предпочитаю сегодня спать одна."
    if persistent.me_neu_loki_true and persistent.me_neu_dr_true and persistent.me_neu_herc_true:
        cs "И не забывай, что это мой старый пациент."
    cs "И если что, я как-нибудь управлюсь."
    show ba em1 uniform with dspr
    ba "Понял."
    hide ba with dissolve
    play sound sfx_close_door_campus_1
    "Шумно топая, физрук покинул помещение."
    cs "Ох и доставил ты нам проблем… {w}пионер."
    "Она прошлась по медпункту и села на край кушетки."
    "И ледяными пальцами схватила моё запястье так, что я на автомате одёрнул руку."
    cs "О, так ты не спишь?"
    scene bg int_aidpost_night
    show cs normal
    show unblink
    "Сделав вид, что только что проснулся, я притворно вздрогнул и открыл глаза."
    me "Виола? А что…"
    play sound sfx_bed_squeak2
    "Я присел на кушетке и осмотрелся."
    "Судя по темноте за окном, уже глубокая ночь, а по немного помятому виду медсестры — её подняли совсем недавно."
    "Пока я старательно делал вид ничего не понимающего пациента, Виола успела налить в стакан воды и протянула его мне."
    cs "Как самочувствие?"
    me "Немного болит голова."
    cs "Сейчас дам таблетку."
    show cs normal at left with move
    "Она улыбнулась и отошла к шкафчику с лекарствами."
    "Я же судорожно размышлял."
    th "Что бы она ни дала — нельзя это пить! Если это будет снотворное, то я проваляюсь до утра."
    th "А мне ну никак нельзя сидеть здесь до утра!"
    "Виола зашуршала блистером, вынимая таблетку из упаковки."
    th "Руки-ноги в порядке. Голова, вроде как, тоже."
    th "Не считая моей внезапной телепортации в медпункт, я всё отлично помнил."
    th "А значит, надо бежать! {w}И ни в коем случае больше не спать!"
    cs "Вот, выпей."
    "Таблетка упала в раскрытую ладонь. Я закинул её в рот, аккуратно спрятал под язык и сделал небольшой глоток."
    me "Спасибо. Я пожалуй посплю ещё немного…"
    hide cs with dissolve
    stop music fadeout 2
    "Широко зевнув, я накинул на себя одеяло и отвернулся к стене, попутно выплюнув таблетку."
    play sound sfx_7dl["light_on_off"]
    play music music_7dl["sky_feather"]
    scene bg int_aidpost_no_light_night_7dl with dissolve
    "Пять минут притворного сопения и, наконец, Виола не выдержала, погасила свет и пошла досыпать свой сон в изолятор."
    "Выждав для уверенности ещё немного, я поднялся с кушетки и на цыпочках прокрался к двери."
    "Заперто!"
    play sound sfx_open_window
    stop ambience fadeout 2
    "Поблагодарив Рандом за то, что здание медпункта одноэтажное, я тихонько отворил щеколду и вылез в окно."
    play ambience ambience_camp_center_night fadein 3
    scene bg ext_aidpost_night with dissolve
    "Но как бы я ни старался сделать это тихо, меня ждал провал."
    "Потому что прямо под окном я спустился в небольшой кустик крапивы!"
    with vpunch
    me "Твою-ж!"
    scene bg ext_houses_night_7dl at running with dissolve
    "Не сомневаясь в том, что меня услышали, я побежал. {w}Снова."
    th "К воротам! К воротам!"
    scene bg ext_clubs_night with fade
    "Лагерь спал, поэтому я бежал не особо скрываясь. И уже через пару минут стоял перед пунктом назначения."
    "Конечно же их закрыли, но что мне стоит перелезть?"
    "Я вставил ногу в прорезь ворот, по форме повторяющую звезду, и подтянулся."
    dreamgirl "Ч-что… {w}Что ты делаешь?"
    th "О, проснулся наконец!"
    dreamgirl "Чувак, ты не должен этого делать!"
    th "А что я должен? Пинком под зад и назад к мамочке?"
    dreamgirl "О чём ты?"
    "Я подтянулся ещё выше и схватился за верхний край."
    th "Так вот решил наш бравый физрук! Отправить меня обратно!"
    if loki:
        th "К моей мамочке! Интересно, а сколько ей, моей мамочке?"
    else:
        th "Интересно, как он это провернёт, а?"
        th "У вас тут изобрели воскрешение?"
    "Я громко расхохотался. Мне вдруг стало плевать на всё!"
    th "Зачем я здесь? Где я вообще?"
    th "Почему у меня внезапно так сильно заболела голова?"
    dreamgirl "Успокойся, чувак, ты всё неправильно понимаешь!"
    "Ловким движением я перескочил на кирпичный забор, подтянулся и, наконец, выпрямился во весь рост."
    dreamgirl "Убегать бессмысленно!"
    th "Да, ты прав. Это бессмысленно."
    "Я смотрел назад, на спящий лагерь. {w}Я смотрел вперёд, на черноту горизонта, который не сулил мне ничего хорошего."
    "Моя глупая детская наивность, попытка убежать — это всё ерунда."
    "Мне дали шанс, и я его просрал. И теперь мне нет места здесь."
    dreamgirl "Ещё не поздно всё исправить!"
    th "Поздно. Уже давно поздно."
    th "Я просто хочу домой…"
    dreamgirl "Стой! Стой!"
    show blink
    "Глаза закрылись сами собой. А за темнотой пришло ощущение падения."
    $ alt_pause (0.7)
    play sound sfx_bodyfall_1
    show black
    $ alt_pause(2.0)
    ...
    if loki:
        ...
        with Dissolve(1.5)
        pause(1.5)
        "Продлилось оно недолго, ведь уже через пару мгновений я ощутил спиной мягкую, но очень холодную поверхность."
        th "Ах да, точно…"
        "До ушей донёсся скрип снега под ногами — палачи сделали свою работу и им незачем было стоять рядом."
        "Дышать было тяжело, но я попытался. И сразу же ощутил холодное дыхание смерти."
        th "Как я мог забыть?"
        "В уголках глаз стояли не слёзы, нет. Это были кристаллы. И я был уже достаточно холоден для того, чтобы их растопить."
        "Я мог только улыбаться, проматывая в голове этот забавный сон. Или его впору называть предсмертной галлюцинацией?"
        "Как бы то ни было, я рад, что побывал там. Хотя бы одним глазком, совсем чуть-чуть, посмотрел на то, чего мне так не хватало."
        "Сознание угасало, но я успел ещё представить себе лицо криминалиста, который будет меня осматривать."
        "Улыбка, которую я всеми силами держал на лице — испугает его или повеселит?"
        "Очень жаль, но мне не суждено узнать этого."
        "Как и того, был ли этот лагерь реальным."
        "Но всё равно спасибо, Демиург."
    else:
        play music "<to 52.94>" + music_7dl["herc_death"] fadein 5
        ...
        with Dissolve(1.5)
        pause(1.5)
        "Любое падение рано или поздно должно заканчиваться."
        "Вот и я приготовился к жёсткому приземлению, но почему-то боли не ощутил."
        "Может, пуля задела ту часть мозга, что отвечает за эту самую боль?"
        "Где-то далеко кричала женщина. Прозвучал ещё один громкий хлопок."
        th "Умереть из-за бутылки водки…"
        th "Убежать из лагеря из-за такой ерунды…"
        "Слушалось бы тело — на моём лице точно была бы самая широкая улыбка из возможных."
        "Кошку жалко. {w}Зину жалко."
        "Но я всё равно улыбался бы."
        "Потому что искренне считаю это крайне забавным."
        "Так и должен умирать неудачник вроде меня."
        "Так и должен умирать человек, бездарно потративший свой «второй» шанс. {w}Как и все шансы до этого."
...
label alt_day2_slot_mi:
    ...
    "В правое плечо бил бриз, в голове было пусто и как-то щемяще."
    "Отношения с водой у меня всегда были совершенно особенные. Когда говорят, что человек может смотреть на первостихию бесконечно, я вспоминаю именно воду."
    "Потому что лично мне огонь душу не чистит, и земля с ветром — тоже."
    ...
    if herc:
        ...
        "Как-то никогда ничего не трогало. Наверное, потому у меня так толком и не сложились отношения ни с одной девушкой…"
    ...
    "И глаза, глаза-то какие — шальные, шалые. Глянешь в них — и ух! — будто с водяной горки, с пятнадцати метров вниз летишь!"
    "Не щурится, не хмурится, а всё равно, смотрит так, будто она — это добрейший батюшка Лаврентий Палыч, и сейчас тебя, контру, давить будет."
    scene cg d2_miku_piano2
    show anim_grain
    with dissolve
    "Вся эта история дурацкая, с роялем и губной гармошкой."
    "Она встала и отряхнулась, а мне стыдно стало. Будто это она меня, а не я её буквой «зю» под роялем застукал."
    ...
    menu:
        "Я неприязненно дёрнул плечом":
            ...
            scene cg day2_mi_beach3_7dl
            $ persistent._seen_images["cg", "day2_mi_beach3_gal"] = True
            show day2_beach_mi_shy3
            show day2_beach_me_normal3
            with dissolve
            ...
            scene cg day2_mi_beach3_7dl
            show day2_beach_mi_angry3
            show day2_beach_mi_tears3
            show day2_beach_me_normal3
            with dissolve
            ...
            scene cg day2_mi_beach3_7dl
            show day2_beach_mi_sad3
            show day2_beach_me_sad3
            with dissolve
            ...
            scene cg day2_mi_beach4_7dl
            $ persistent._seen_images["cg", "day2_mi_beach4_gal"] = True
            show day2_beach_mi_surp_eye4
            if alt_day2_mi_kumuhimo == 2:
                show day2_beach_cumihimo4
                with dissolve
            ...
            scene cg day2_mi_beach4_7dl
            show day2_beach_mi_normal_eye4
            if alt_day2_mi_kumuhimo == 2:
                show day2_beach_cumihimo4
            with dissolve
        ...
        "Я замер":
            ...
            if ('music_club' in list_voyage_7dl):
                ...
                mi "Но разве я так непонятно говорю, мы когда в Японии жили, Па хвалил меня за то, как я управляюсь с идиомами, а тут ты критикуешь. Но критика же всегда полезна. Так что ты хотел сказать?"
            ...
            scene cg day2_mi_beach1_7dl
            $ persistent._seen_images["cg", "day2_mi_beach1_gal"] = True
            show day2_beach_mi_smile1
            show day2_beach_me_normal1
            with dissolve
            with dissolve
            mi "Я поняла, Сёмочка."
            "Прощебетала она."
            ...
            scene cg day2_mi_beach1_7dl
            show day2_beach_mi_2smile1
            show day2_beach_me_sad1
            with dissolve
            mi "Ничего?"
            ...
            scene cg day2_mi_beach1_7dl
            show day2_beach_mi_normal1
            show day2_beach_me_sad1
            with dissolve
            mi "Нет."
            "Удручающе честно ответила Мику."
            scene cg day2_mi_beach1_7dl
            show day2_beach_mi_sad1
            show day2_beach_me_sad1
            with dissolve
            ...
            menu:
                "Верни голову на место":
                    ...
                    scene cg day2_mi_beach1_7dl
                    show day2_beach_mi_2smile1
                    show day2_beach_me_smile1
                    with dissolve
                    mi "Очень."
                    "И как после этих слов можно было поступить иначе?"
                    scene cg day2_mi_beach2_7dl
                    $ persistent._seen_images["cg", "day2_mi_beach2_gal"] = True
                    show day2_beach_mi_surp2
                    with dissolve
                    "Согнутый указательный палец под подбородок, запрокидывая девочку лицом навстречу ищущим губам."
                    scene cg day2_mi_beach2_7dl
                    show day2_beach_mi_cl2
                    with dissolve
...
label alt_day2_un_loki_date:
    ...
    me "Да вещи! Только на толстую тётку раза в три меня крупнее. Нет, если нужда будет, я смогу спрятаться от дождя, развесив одну из её розовых плащ-кофточек…"
    ...
    "Я не знаю, зачем придумал всю эту историю, но она пришлась девочке по душе."
    "Лена смеялась, представляя себе, как я сижу под кустом, а надо мной висит огромная розовая кофта размером с чехол для танка."
    ...
    if alt_day2_us_escape:
        ...
        me "Короткий?"
        un "Ближний."
    me "Сейчас возьмём лодку…"
    if alt_day2_us_escape:
        "Мои мышцы протестующе застонали, но я приказал им заткнуться."
    un "Нет… Вернее, я не против, но… Не сегодня."
    ...
    dreamgirl "Не хотелось бы вторгаться в ваше интимное, но не слишком ли она близко?"
    th "Заткнись."
    dreamgirl "Я серьёзно. Если вдруг захочешь её поцеловать, тебе не придётся ничего делать."
    dreamgirl "Достаточно просто обернуться, и…"
    "И чёрт бы меня дёрнул сделать ровно то, что озвучил этот внутренний придурок!"
    ...
    un "Мммм…"
    th "Сейчас она осознает что произошло и нашей милой прогулочке конец!"
    th "Спасибо тебе огромное, хренов советчик!"
    dreamgirl "Каков наглец, а? Я же тебя предупреждал, а не давал инструкций к действию!"
    ...
    with Dissolve(1.5)
    pause(2)
    scene cg d2_un_knees_7dl with Dissolve(1.6)
    pause(1)
    ...
    un "Что ж… Я из провинциального городка неподалёку отсюда. Примерно в паре часов езды."
    "Подобное описание не давало ровным счётом никаких подсказок, но я решил не настаивать. Успеем ещё обменяться адресами."
    ...
    me "Да причём здесь образование? Я же о твоём призвании."
    scene cg d2_un_knees3_7dl with dissolve
    ...
    "Мне почему-то казалось очень важным, чтобы хотя бы эта девочка не профукала дарованный ей шанс."
    scene cg d2_un_knees1_7dl with Dissolve(0.45)
    un "Да, но…"
    me "Не зарывай талант в землю. {w}Подавай в худ, пускай тебе руку набивают."
    scene cg d2_un_knees2_7dl with Dissolve(0.45)
    ...
    scene cg d2_un_knees4_7dl with Dissolve(0.55)
...
label alt_day2_un_herc_date:
    ...
    th "Для хорошего художника каждая его картина — это личное переживание. Он проживает сюжет, делает выводы, а потом выражает всё на холсте."
    ...
    th "Возьми себя в руки!"
    th "Держи в своей пустой голове, что ты ступаешь на тонкий лёд."
    th "И весь твой незначительный жизненный опыт тебе не в помощь здесь."
    th "Потому что он абсолютно о другом."
    ...
    "И в голове мелькнула мысль о том, что расстояние до губ девочки не превышает десяти сантиметров."
    me "В-всё… т-так…"
    if (alt_day2_convoy == 'un') and ('library' in list_voyage_7dl):
        ...
        dreamgirl "Впрочем, этого следовало ожидать."
        "Противно было слушать глумление моего внутреннего голоса, но я предпочёл игнорировать его."
...
label alt_day2_slot_un:
    ...
    "Воланчик взлетел метров на семь, добрался до пика и, развернувшись, устремился вниз."
    ...
    "Не знаю, откуда в ней взялся этот неописуемый страх, но мне пришлось потратить ещё пару минут на поглаживания, поддерживания и увещевания."
    ...
    "Я кивнул на воланчик в её руке."
    ...
    dreamgirl "Я просто понять не могу — это тебе на слепых баб везёт, или ты ищешь их специально, чтобы рожу рассмотреть не успе… {w}Стоп, погоди…"
    th "Чего?"
    dreamgirl "Кхм-кхм. Ничего. Просто неудачная шутка."
    th "А у тебя бывают другие?"
    show un smile sport with dspr
...
label alt_day2_slot_dv:
    ...
    if not alt_day2_dv_chased:
        ...
        "Я не большой фанат купаний, лет до четырнадцати я вообще не умел плавать, и большинство моих купаний ограничивалось лежанием на воде или маканием ног…"
...
label alt_day2_slot_sl:
    ...
    me "Как можно весь день носиться по лагерю и всем помогать, да ещё с такой улыбочкой, словно тебе ни капельки не сложно?"
...
label alt_day2_slot_mt:
    ...
    dreamgirl "Ну, погоди!"
    scene cg d2_mt_resort_7dl with dissolve
    ...
    $ persistent.sprite_time = "night"
    $ night_time()
    scene cg d2_mt_resort_night_7dl with dissolve2
    ...
    "Она одарила меня быстрой улыбкой и выпорхнула на улицу."
    hide mt with dissolve
    ...
    scene
    $ renpy.show("cg d1_me_mirror_casual2_7dl", what = Sepia("cg d1_me_mirror_casual2_7dl"))
    show anim_grain
    with flash
    ...
    mt "А завтра ты включишь своего «ходока по жизни», и не угнаться за тобой, пока ты копишь впечатления."
